...
public class UserPassServiceImpl  implements IUserPassService {
       ...
       filters.add(new PrefixFilter(rowPrefix));
        //2、基于列单元值的过滤器，找到未使用的优惠券
         ...
         for (Pass pass:passes){
            ...
            if(null==merchants){
                log.error("Merchants null",pass.getTemplateId());
                continue;
            }
    ...
    private Map<String, PassTemplate>  buildPassTemplateMap(List<Pass>  passes) throws Exception{
        ...
        byte[]  START=Bytes.toBytes(Constants.PassTemplateTable.STRAT);
        ...
        //构造 PassTemplateId  -> PassTemplate Object 的Map,用于构造 PassInfo
        ...
        for (Result item : templateResults) {
            ...
            passTemplate.setLimit(Bytes.toLong(item.getValue(FAMILY_B, LIMIT)));
    ...
    /**
     * <h2>通过获取的 PassTemplate 对象构造Merchants Map</h2>
