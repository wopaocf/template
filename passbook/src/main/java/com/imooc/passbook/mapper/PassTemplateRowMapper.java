...
public class PassTemplateRowMapper implements RowMapper<PassTemplate> {
    ...
    private static  byte[]   BACKGROUD=Constants.PassTemplateTable.BACKGROUD.getBytes();
    ...
    private static  byte[]  STRAT=Constants.PassTemplateTable.STRAT.getBytes();
    ...
    public PassTemplate mapRow(Result result, int rowNum) throws Exception {
           ...
           passTemplate.setBackgroud(Bytes.toInt(result.getValue(FAMILY_B,BACKGROUD)));
           ...
           passTemplate.setStart(DateUtils.parseDate(Bytes.toString(result.getValue(FAMILY_C,STRAT)),patterns));
